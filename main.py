...
if "result_text" in st.session_state:
    ...
    ts = st.session_state.get("result_ts")
    base_name = f"문항_개발_결과_{ts}" if ts else "문항_개발_결과"

    st.markdown("### 📝 개발된 시나리오 기반 문항 세트")
    ...
    pdf_bytes = generate_pdf_bytes(result_text)
    if pdf_bytes:
        st.download_button(
            "PDF로 다운로드",
            data=pdf_bytes,
        ...
        st.caption("PDF 내보내기를 위해서는 reportlab 설치가 필요합니다: pip install reportlab")

    png_bytes = generate_png_bytes(result_text)
    if png_bytes:
        st.download_button(
            "PNG 이미지로 다운로드",
            data=png_bytes,
        ...
        st.caption("이미지 내보내기를 위해서는 pillow 설치가 필요합니다: pip install pillow") 
